...
const formState = {
  uiFlags: useMapGetter('ai_agentInboxes/getUIFlags'),
  inboxes: useMapGetter('inboxes/getInboxes'),
  ai_agentInboxes: useMapGetter('ai_agentInboxes/getRecords'),
};
...
const inboxList = computed(() => {
  const ai_agentInboxIds = formState.ai_agentInboxes.value.map(inbox => inbox.id);

  return formState.inboxes.value
    .filter(inbox => !ai_agentInboxIds.includes(inbox.id))
    .map(inbox => ({
      value: inbox.id,
      label: inbox.name,
    }));
