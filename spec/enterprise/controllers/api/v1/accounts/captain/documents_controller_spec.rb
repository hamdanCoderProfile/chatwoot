...
RSpec.describe 'Api::V1::Accounts::Captain::Documents', type: :request do
  ...
  let(:topic2) { create(:captain_topic, account: account) }
  ...
  describe 'GET /api/v1/accounts/:account_id/captain/documents' do
    ...
    context 'when it is an agent' do
      context 'when no filters are applied' do
        before do
          create_list(:captain_document, 30, topic: topic, account: account)
        end
      ...
      context 'when filtering by topic_id' do
        before do
          create_list(:captain_document, 3, topic: topic, account: account)
          create_list(:captain_document, 2, topic: topic2, account: account)
        end
        ...
        it 'returns empty array when topic has no documents' do
          new_topic = create(:captain_topic, account: account)
          get "/api/v1/accounts/#{account.id}/captain/documents",
      ...
      context 'with pagination and topic filter combined' do
        before do
          create_list(:captain_document, 30, topic: topic, account: account)
          create_list(:captain_document, 10, topic: topic2, account: account)
        end
  ...
  describe 'POST /api/v1/accounts/:account_id/captain/documents' do
    ...
    context 'when it is an admin' do
      ...
      context 'with limits exceeded' do
        before do
          create_list(:captain_document, 5, topic: topic, account: account)
  ...
  describe 'DELETE /api/v1/accounts/:account_id/captain/documents/:id' do
    ...
    context 'when it is an agent' do
      let!(:document_to_delete) { create(:captain_document, topic: topic) }
